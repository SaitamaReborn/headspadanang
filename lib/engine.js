...
<details class="lang"><summary>EN</summary><div>${LANGS.map(l=>`<a href="${l.path}">${l.native}</a>`).join('')}</div></details>
  ...
  /* Every other link on these pages sends the reader away — to a Google listing,
     to a competitor's own site. Across 55 sampled pages there was no phone
     number, no booking link and no form for the one house the guide actually
     recommends: 329 queries ranked in the top ten and returned nothing. This row
     is the missing step. It carries the real number, it sits after the answer
     rather than in front of it, and the disclosure travels with it.
     rel="sponsored" because the relationship is commercial and stated: the one
     editorially-weighted link stays on the pick's own profile page, where it
     belongs, instead of being stamped across four hundred pages. */
  const REACH=(()=>{
  ...
  /* The same step, one line, for the templates that carry no editor's note —
     the home page, the full directory, the journal, the static pages. */
  const REACH_LINE=(()=>{
    const p=featured, P=cfg.PARTNER||{};
    if(!p||!P.site) return '';
    const shown=P.phone||p.phone||'', tel=(P.phoneRaw||shown).replace(/[^+\d]/g,'');
    return `<p class="foot-reach">This guide's pick is <a href="${LISTING.path}${p.slug}/">${esc(p.name)}</a>`
      +`${tel?` · <a href="tel:${tel}">${esc(shown)}</a>`:''}`
      +`${P.whatsapp?` · <a href="${P.whatsapp}" rel="noopener sponsored">WhatsApp</a>`:''}`
      +` · <a href="${P.site}" rel="noopener sponsored">${esc(P.siteLabel||'Menu & prices')}</a>`
      +` — it works commercially with this guide, and <a href="/about/">how we rank</a> says exactly how.</p>`;
  })();
  const pick=(compact)=>{
