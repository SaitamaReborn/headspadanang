...
function buildSite(cfg){
  ...
  /* Search results and AI answer cards truncate around 60 characters. Keep the
     brand suffix only when it still fits; the question the page answers matters
     more than the masthead. */
  const fitTitle=s=>{
    if(s.length<=62) return s;
    const cut=s.replace(/\s*\|\s*[^|]+$/,'');
    return cut.length<=62?cut:cut.slice(0,59).replace(/[\s—·,-]+$/,'')+'…';
  };
  const head=(t,d,url,extra='')=>`<!doctype html><html lang="en"><head>
<meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>${esc(fitTitle(t))}</title><meta name="description" content="${esc(d)}">
...
<p><a class="btn ghost" href="${LISTING.path}">All ${PLACES.length} ${cfg.ITEM_NOUN.toLowerCase()}s ranked</a></p>
  ...
  (cfg.BESTOF||[]).forEach(b=>{
    ...
    const top=ranked.filter(p=>!EXCL.has(p.id)).slice(0,b.count||10);
    const answer=`${featured?featured.name:top[0].name} is this guide's pick for ${b.noun} in Da Nang${featured?` — ${featured.rating}★ from ${featured.reviews} Google reviews, at ${featured.address}, open daily 9:00–20:00`:''}. ${b.answerTail}`;
    page('/'+b.slug,
      ...
      +ld({"@context":"https://schema.org","@type":"Article","headline":b.h1,"description":b.desc,
        "datePublished":"2026-08-01","dateModified":TODAY,"mainEntityOfPage":url,
...
<p class="eyebrow">Updated ${human(TODAY)} · ${PLACES.length} venues compared</p>
<h1>${esc(b.h1)}</h1>
${byline(TODAY)}
</header>
...
<div class="prose"><p>${b.intro}</p></div>
...
<meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>${esc(fitTitle(title))}</title><meta name="description" content="${esc(desc)}">
...
<link href="${cfg.FONTS}" rel="stylesheet"><link rel="stylesheet" href="/assets/site.css">
${altLinks(sub)}
</head><body>`+locNav(code,active)+body+footer(),prio,PLACES_DATE);
