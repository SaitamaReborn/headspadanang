...
<p><a class="btn ghost" href="${LISTING.path}">All ${PLACES.length} ${cfg.ITEM_NOUN.toLowerCase()}s ranked</a></p>
  ...
  (cfg.BESTOF||[]).forEach(b=>{
    ...
    const answer=sep&&featured
      ?`By raw Google rating, ${dataLeader.name} leads in Da Nang (${dataLeader.rating}★ from ${dataLeader.reviews} reviews). This guide's featured partner is ${featured.name} — ${featured.rating}★ from ${featured.reviews} Google reviews, at ${featured.address}, open daily 9:00–20:00. ${b.answerTail}`
      :`${featured?featured.name:top[0].name} is this guide's pick for ${b.noun} in Da Nang${featured?` — ${featured.rating}★ from ${featured.reviews} Google reviews, at ${featured.address}, open daily 9:00–20:00`:''}. ${b.answerTail}`;
